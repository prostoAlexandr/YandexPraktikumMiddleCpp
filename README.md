...
После установки Docker переходите к разделу: `До начала использования Docker контейнера: Настройка переменных окружения`.
...
**Важно!** Для запуска benchmark тестов необходимо запустить собранное приложение вручную `/build/src/benchmark_tests` или используя команду `Launch Release benchmark_tests` без режима отладки (для этого нажмите `Ctrl + F5`)
